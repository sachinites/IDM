...
timer_register_app_event(wheel_timer_t *wt,
	...
	wheel_timer_elem_t *wt_elem = 
		timer_create_new (wt, call_back, arg, arg_size, time_interval, is_recursive);

	if (!wt_elem) return NULL;
    _timer_reschedule(wt, wt_elem, time_interval, WTELEM_CREATE);
    return wt_elem;
}

wheel_timer_elem_t * 
timer_create_new (wheel_timer_t *wt, 
		   app_call_back call_back, 
		   void *arg,
		   int arg_size, 
		   int time_interval, 
...
timer_start (wheel_timer_elem_t *wt_elem) {
	 ...
	 _timer_reschedule(wt_elem->wt, wt_elem, wt_elem->time_interval, WTELEM_CREATE);
}
